  ...
  if (activeScreen === 'home') {
    return (
      <div className="min-h-screen w-full bg-[#0d0926] text-white flex flex-col justify-between selection:bg-pink-500 selection:text-white p-4 md:p-8 relative overflow-hidden font-sans bg-[radial-gradient(#ec4899_2px,transparent_2px)] [background-size:28px_28px]">
        ...
        <header className="w-full max-w-6xl mx-auto flex items-center justify-between py-4 px-6 bg-[#16123b]/95 backdrop-blur border-4 border-black rounded-3xl shadow-[8px_8px_0px_#ec4899] z-10">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 bg-gradient-to-tr from-yellow-300 via-pink-500 to-cyan-400 border-3 border-black rounded-2xl flex items-center justify-center font-black text-black text-2xl shadow-[3px_3px_0px_#000] rotate-[-3deg]">
              🤪
            </div>
            <div>
              <span className="font-black text-xl md:text-2xl tracking-wider text-transparent bg-clip-text bg-gradient-to-r from-yellow-300 via-pink-400 to-cyan-400 uppercase drop-shadow-[2px_2px_0px_#000]">
                ESQUIZOFRENIA VIRTUAL
              </span>
              <p className="text-[11px] font-mono font-bold text-cyan-300 flex items-center gap-1">
                <span>🎨 Multiverso Dibujado de Física</span>
                <span className="bg-yellow-400 text-black px-1.5 py-0.2 text-[9px] rounded font-black">v3.5</span>
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            {userProfile && (
              <div
                onClick={() => {
                  ...
                  setRegGrade(userProfile.grade);
                  setShowRegisterModal(true);
                }}
                className="hidden sm:flex items-center gap-1.5 bg-pink-500 text-white px-3 py-1.5 rounded-xl border-2 border-black shadow-[2px_2px_0px_#000] text-xs font-black cursor-pointer hover:bg-pink-600 transition-all rotate-[-1deg]"
                title="Editar perfil de registro"
              >
            ...
            <button
              ...
              className="px-5 py-2.5 bg-gradient-to-r from-yellow-300 via-pink-400 to-cyan-400 border-3 border-black text-black font-black text-sm uppercase rounded-2xl shadow-[4px_4px_0px_#000] hover:shadow-[7px_7px_0px_#00E5FF] hover:-translate-y-1 active:translate-x-1 active:translate-y-1 transition-all flex items-center gap-2 cursor-pointer"
            >
              <Zap className="w-5 h-5 fill-black text-black animate-bounce" />
              <span>¡Entrar al Cómic!</span>
            </button>
          </div>
        </header>
          ...
          <section className="lg:col-span-3 space-y-6">
            ...
            {worldMode === 'world2' && (
              <div className="space-y-6">
                ...
                {/* ========================================================================= */}
                {/* DEDICATED ARRIVAL TELEMETRY & KINEMATIC PROOF BOARD                       */}
                {/* ========================================================================= */}
                <div className="bg-[#0b0e1b] border-4 border-amber-400/90 shadow-[0_0_25px_rgba(251,191,36,0.3)] p-5 rounded-lg space-y-4 text-white">
                  <div className="flex flex-wrap items-center justify-between gap-3 border-b-2 border-amber-500/40 pb-3 font-mono">
                    <div className="flex items-center space-x-2">
                      <Award className="w-5 h-5 text-amber-400 animate-pulse" />
                      <h4 className="font-black text-sm uppercase text-amber-300 tracking-wider">
                        Cronometraje Oficial & Registro de Verdadera Llegada
                      </h4>
                    </div>
                    <div className="flex items-center space-x-2 text-xs">
                      <span className="px-2.5 py-1 bg-amber-400/20 text-amber-300 font-bold border border-amber-400/40 rounded">
                        Distancia de Pista: <strong>L = {world2TrackLength} m</strong>
                      </span>
                    </div>
                  </div>

                  {/* Registered Podiums Grid */}
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 font-mono text-xs">
                    {/* Podium 1: MUA */}
                    {(world2SubMode === 'both' || world2SubMode === 'mua') && (
                      <div className={`p-4 rounded-lg border-2 transition-all ${
                        cubePhysics.winner === 'MUA' || world2SubMode === 'mua'
                          ? 'bg-purple-950/60 border-purple-400 shadow-[0_0_15px_rgba(168,85,247,0.3)]'
                          : 'bg-purple-950/20 border-purple-800'
                      }`}>
                        <div className="flex items-center justify-between border-b border-purple-500/40 pb-2 mb-2">
                          <span className="font-black uppercase text-sm text-purple-300 flex items-center gap-1.5">
                            {cubePhysics.winner === 'MUA' ? '🥇 1° LUGAR (MUA)' : '🥈 2° LUGAR (MUA)'}
                          </span>
                          <span className="px-2 py-0.5 bg-purple-500/30 text-purple-200 text-[10px] font-bold rounded">
                            a = {muaAcc} m/s² | v₀ = {muaV0} m/s
                          </span>
                        </div>

                        <div className="space-y-1.5 text-[11px]">
                          <div className="flex justify-between">
                            <span className="text-slate-300">⏱️ Tiempo Exacto de Llegada:</span>
                            <strong className="text-purple-300 text-sm font-black">
                              {cubePhysics.mua.timeToFinish.toFixed(3)} s
                            </strong>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-slate-300">⚡ Velocidad al Cruzar Meta (v_final):</span>
                            <strong className="text-purple-200">
                              {cubePhysics.mua.vFinish.toFixed(2)} m/s ({(cubePhysics.mua.vFinish * 3.6).toFixed(1)} km/h)
                            </strong>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-slate-300">📊 Velocidad Media (L / t):</span>
                            <strong className="text-purple-200">
                              {cubePhysics.mua.avgSpeed.toFixed(2)} m/s
                            </strong>
                          </div>
                          <div className="pt-2 border-t border-purple-500/30 text-[10px] text-purple-200/90 font-mono">
                            Fórmula: <code className="text-amber-300">t = (-v₀ + √(v₀² + 2aL)) / a</code>
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    {(world2SubMode === 'both' || world2SubMode === 'mru') && (
                      <div className={`p-4 rounded-lg border-2 transition-all ${
                        cubePhysics.winner === 'MRU' || world2SubMode === 'mru'
                          ? 'bg-cyan-950/60 border-cyan-400 shadow-[0_0_15px_rgba(56,189,248,0.3)]'
                          : 'bg-cyan-950/20 border-cyan-800'
                      }`}>
                        <div className="flex items-center justify-between border-b border-cyan-500/40 pb-2 mb-2">
                          <span className="font-black uppercase text-sm text-cyan-300 flex items-center gap-1.5">
                            {cubePhysics.winner === 'MRU' ? '🥇 1° LUGAR (MRU)' : '🥈 2° LUGAR (MRU)'}
                          </span>
                          <span className="px-2 py-0.5 bg-cyan-500/30 text-cyan-200 text-[10px] font-bold rounded">
                            v = {mruV} m/s (Constante, a = 0)
                          </span>
                        </div>
                        ...
                        <div className="space-y-1.5 text-[11px]">
                          <div className="flex justify-between">
                            ...
                            <strong className="text-cyan-300 text-sm font-black">
                              {cubePhysics.mru.timeToFinish.toFixed(3)} s
                            </strong>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-slate-300">⚡ Velocidad al Cruzar Meta (v_cte):</span>
                            <strong className="text-cyan-200">
                              {cubePhysics.mru.vFinish.toFixed(2)} m/s ({(cubePhysics.mru.vFinish * 3.6).toFixed(1)} km/h)
                            </strong>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-slate-300">📊 Velocidad Media (v_cte):</span>
                            <strong className="text-cyan-200">
                              {cubePhysics.mru.avgSpeed.toFixed(2)} m/s
                            </strong>
                          </div>
                          <div className="pt-2 border-t border-cyan-500/30 text-[10px] text-cyan-200/90 font-mono">
                            Fórmula: <code className="text-amber-300">t = L / v</code>
                          </div>
                        </div>
                      </div>
                    )}
                  ...
                  {/* Comparative Margin of Victory (if both mode) */}
                  {world2SubMode === 'both' && (
                    <div className="bg-[#131728] p-3.5 rounded-lg border border-amber-500/50 flex flex-wrap items-center justify-between gap-3 text-xs font-mono">
                      <div>
                        <span className="text-amber-400 font-bold uppercase block text-[10px]">
                          Margen Oficial de Victoria en Meta:
                        </span>
                        <p className="text-slate-200 text-xs">
                          {cubePhysics.winner === 'MUA' && (
                            <>MUA cruzó la meta <strong className="text-amber-300">{cubePhysics.timeDiff.toFixed(3)} segundos antes</strong> que MRU, dejando a MRU a una distancia rezagada de <strong className="text-cyan-300">{cubePhysics.marginDistanceMeters.toFixed(1)} m</strong> de la meta.</>
                          )}
                          {cubePhysics.winner === 'MRU' && (
                            <>MRU cruzó la meta <strong className="text-amber-300">{cubePhysics.timeDiff.toFixed(3)} segundos antes</strong> que MUA, dejando a MUA a una distancia rezagada de <strong className="text-purple-300">{cubePhysics.marginDistanceMeters.toFixed(1)} m</strong> de la meta.</>
                          )}
                          {cubePhysics.winner === 'TIE' && (
                            <>Ambos cruzaron la meta exactamente al mismo instante con una diferencia menor a 0.005 s.</>
                          )}
                        </p>
                      </div>
                      <div className="bg-amber-400 text-[#0b0e1b] font-black px-3 py-1.5 rounded shadow text-xs">
                        Δt = {cubePhysics.timeDiff.toFixed(3)} s
                      </div>
                    </div>
                  )}
                </div>
            ...
            {worldMode === 'world3' && (
              <div className="space-y-6">
                ...
                {/* ========================================================================= */}
                {/* LA VERDADERA CALCULACIÓN: DESGLOSE FÍSICO Y MATEMÁTICO COMPLETO PASO A PASO */}
                {/* ========================================================================= */}
                <div className="bg-[#0b0e1b] border-4 border-amber-400 p-5 rounded-lg space-y-5 text-amber-100 shadow-[0_0_25px_rgba(251,191,36,0.25)]">
                  <div className="flex flex-wrap items-center justify-between gap-3 border-b-2 border-amber-500/40 pb-3">
                    <div className="flex items-center space-x-3">
                      <span className="p-2 bg-amber-400 text-[#0b0e1b] rounded font-black text-sm">
                        📐
                      </span>
                      <div>
                        <h4 className="font-black text-base uppercase text-white font-mono tracking-wider">
                          La Verdadera Calculación Cinemática y Dinámica
                        </h4>
                        <p className="text-xs text-amber-300 font-mono">
                          Demostración matemática rigurosa con sustitución de valores numéricos en vivo.
                        </p>
                      </div>
                    </div>

                    <button
                      onClick={() => setW3ShowStepByStep(!w3ShowStepByStep)}
                      className="px-3 py-1.5 bg-amber-500/20 hover:bg-amber-500/30 text-amber-300 border border-amber-400 rounded text-xs font-mono font-bold transition-all"
                    >
                      {w3ShowStepByStep ? 'Ocultar Fórmulas Detalladas' : 'Ver Fórmulas Detalladas'}
                    </button>
                  </div>

                  {/* 1. COMPARISON CARDS: GUMBALL VS DARWIN REAL DATA */}
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {/* Card 1: Gumball / Roca */}
                    <div className="bg-[#131728] border-2 border-cyan-400/70 p-4 rounded-lg space-y-3 font-mono">
                      <div className="flex items-center justify-between border-b border-cyan-500/30 pb-2">
                        <span className="font-black text-cyan-300 text-sm flex items-center gap-1.5">
                          🐱 Gumball ({ffRockMass} kg) - {ffVacuum ? 'MUA Puro' : 'Arrastre Mínimo'}
                        </span>
                        <span className="text-[10px] bg-cyan-400/20 text-cyan-200 px-2 py-0.5 rounded border border-cyan-400/40">
                          Torre 1
                        </span>
                      </div>

                      <div className="space-y-2 text-xs">
                        <div className="flex justify-between items-center bg-[#0a0f1d] p-2 rounded">
                          <span className="text-slate-300">⏱️ Tiempo de Caída (t):</span>
                          <strong className="text-cyan-300 text-sm font-black">
                            {w3Rock.impactTime.toFixed(3)} s
                          </strong>
                        </div>
                        <div className="flex justify-between items-center bg-[#0a0f1d] p-2 rounded">
                          <span className="text-slate-300">⚡ Velocidad al Impactar (v_impacto):</span>
                          <strong className="text-amber-300 font-black">
                            {(Math.sqrt(2 * ffGravity * ffHeight) * (ffVacuum ? 1 : 0.99)).toFixed(2)} m/s ({(Math.sqrt(2 * ffGravity * ffHeight) * 3.6 * (ffVacuum ? 1 : 0.99)).toFixed(1)} km/h)
                          </strong>
                        </div>
                        <div className="flex justify-between items-center bg-[#0a0f1d] p-2 rounded">
                          <span className="text-slate-300">💥 Energía Cinética Impacto (Ec = ½·m·v²):</span>
                          <strong className="text-pink-300 font-bold">
                            {(0.5 * ffRockMass * Math.pow(Math.sqrt(2 * ffGravity * ffHeight) * (ffVacuum ? 1 : 0.99), 2)).toFixed(1)} J ({(0.5 * ffRockMass * Math.pow(Math.sqrt(2 * ffGravity * ffHeight) * (ffVacuum ? 1 : 0.99), 2) / 1000).toFixed(2)} kJ)
                          </strong>
                        </div>
                        <div className="flex justify-between items-center bg-[#0a0f1d] p-2 rounded">
                          <span className="text-slate-300">🏔️ Energía Potencial Inicial (Ep = m·g·h):</span>
                          <strong className="text-purple-300 font-bold">
                            {(ffRockMass * ffGravity * ffHeight).toFixed(1)} J
                          </strong>
                        </div>
                        <div className="flex justify-between items-center bg-[#0a0f1d] p-2 rounded">
                          <span className="text-slate-300">🔨 Fuerza Media de Impacto (F ≈ Δp / 0.05s):</span>
                          <strong className="text-red-300 font-bold">
                            {((ffRockMass * Math.sqrt(2 * ffGravity * ffHeight)) / 0.05).toFixed(0)} N
                          </strong>
                        </div>
                      </div>
                    </div>

                    {/* Card 2: Darwin / Moneda */}
                    <div className="bg-[#131728] border-2 border-orange-400/70 p-4 rounded-lg space-y-3 font-mono">
                      <div className="flex items-center justify-between border-b border-orange-500/30 pb-2">
                        <span className="font-black text-orange-300 text-sm flex items-center gap-1.5">
                          🐟 Darwin ({ffCoinMass} kg) - {ffVacuum ? 'MUA Puro' : 'Freno Terminal MRU'}
                        </span>
                        <span className="text-[10px] bg-orange-400/20 text-orange-200 px-2 py-0.5 rounded border border-orange-400/40">
                          Torre 2
                        </span>
                      </div>
                      ...
                      <div className="space-y-2 text-xs">
                        <div className="flex justify-between items-center bg-[#0a0f1d] p-2 rounded">
                          <span className="text-slate-300">⏱️ Tiempo de Caída (t):</span>
                          <strong className="text-orange-300 text-sm font-black">
                            {w3Coin.impactTime.toFixed(3)} s
                          </strong>
                        </div>
                        <div className="flex justify-between items-center bg-[#0a0f1d] p-2 rounded">
                          <span className="text-slate-300">⚡ Velocidad al Impactar (v_impacto):</span>
                          <strong className="text-amber-300 font-black">
                            {w3Coin.v.toFixed(2)} m/s ({(w3Coin.v * 3.6).toFixed(1)} km/h)
                          </strong>
                        </div>
                        <div className="flex justify-between items-center bg-[#0a0f1d] p-2 rounded">
                          <span className="text-slate-300">💥 Energía Cinética Impacto (Ec = ½·m·v²):</span>
                          <strong className="text-pink-300 font-bold">
                            {(0.5 * ffCoinMass * Math.pow(w3Coin.v, 2)).toFixed(3)} J
                          </strong>
                        </div>
                        <div className="flex justify-between items-center bg-[#0a0f1d] p-2 rounded">
                          <span className="text-slate-300">🏔️ Energía Potencial Inicial (Ep = m·g·h):</span>
                          <strong className="text-purple-300 font-bold">
                            {(ffCoinMass * ffGravity * ffHeight).toFixed(3)} J
                          </strong>
                        </div>
                        {!ffVacuum && (
                          <div className="flex justify-between items-center bg-[#0a0f1d] p-2 rounded">
                            <span className="text-slate-300">💨 Velocidad Terminal Límite (v_terminal):</span>
                            <strong className="text-cyan-300 font-bold">
                              {w3Coin.v_term.toFixed(2)} m/s (MRU)
                            </strong>
                          </div>
                        )}
                        {ffVacuum && (
                          <div className="flex justify-between items-center bg-[#0a0f1d] p-2 rounded">
                            <span className="text-slate-300">🔨 Fuerza Media de Impacto (F ≈ Δp / 0.05s):</span>
                            <strong className="text-red-300 font-bold">
                              {((ffCoinMass * Math.sqrt(2 * ffGravity * ffHeight)) / 0.05).toFixed(1)} N
                            </strong>
                          </div>
                        )}
                      </div>
                  ...
                  {/* 2. STEP-BY-STEP MATHEMATICAL FORMULA PROOF PANEL */}
                  {w3ShowStepByStep && (
                    <div className="bg-[#080c18] border-2 border-amber-500/60 p-4 rounded-lg space-y-4 font-mono text-xs">
                      <h5 className="font-black text-amber-300 uppercase tracking-wider text-xs border-b border-amber-500/30 pb-1.5 flex items-center gap-2">
                        <span>🧪 Demostración Paso a Paso de las Ecuaciones Físicas</span>
                      </h5>

                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-[11px]">
                        {/* Box A: Vacuum Kinematic Proof */}
                        <div className="bg-[#101426] p-3.5 rounded border border-cyan-500/40 space-y-2">
                          <h6 className="font-bold text-cyan-300 uppercase text-[11px]">
                            1. Ecuación en Vacío (MUA Puro - Galileo Galilei):
                          </h6>
                          <div className="p-2 bg-[#05070f] rounded text-slate-200 space-y-1">
                            <p className="text-amber-300 font-bold">Δy = v₀·t + ½·g·t²</p>
                            <p className="text-slate-400">Como v₀ = 0 m/s y parte del reposo:</p>
                            <p className="text-cyan-200">H = ½·g·t² ⟹ t = √( 2·H / g )</p>
                          </div>
                          <div className="p-2 bg-[#05070f] rounded text-amber-200 space-y-1">
                            <p className="font-bold text-slate-300">Sustitución con los valores actuales:</p>
                            <p>t = √( 2 × {ffHeight} m / {ffGravity} m/s² )</p>
                            <p>t = √( {(2 * ffHeight).toFixed(1)} / {ffGravity} ) = √( {( (2 * ffHeight) / ffGravity ).toFixed(3)} )</p>
                            <p className="text-cyan-300 font-black text-xs">
                              ⟹ t = {Math.sqrt((2 * ffHeight) / ffGravity).toFixed(3)} segundos
                            </p>
                          </div>
                          <p className="text-[10px] text-slate-400 italic">
                            💡 Conclusión de Galileo: La masa "m" se cancela en la ecuación diferencial ($m·a = m·g \iff a = g$), por eso Gumball y Darwin caen al mismo instante en el vacío.
                          </p>
                        </div>

                        {/* Box B: Impact Velocity & Energy Conservation */}
                        <div className="bg-[#101426] p-3.5 rounded border border-orange-500/40 space-y-2">
                          <h6 className="font-bold text-orange-300 uppercase text-[11px]">
                            2. Velocidad de Choque y Conservación de Energía:
                          </h6>
                          <div className="p-2 bg-[#05070f] rounded text-slate-200 space-y-1">
                            <p className="text-amber-300 font-bold">v² = v₀² + 2·g·H ⟹ v = √( 2·g·H )</p>
                            <p className="text-slate-400">Conservación: E_potencial = E_cinética (mgh = ½mv²)</p>
                          </div>
                          <div className="p-2 bg-[#05070f] rounded text-amber-200 space-y-1">
                            <p className="font-bold text-slate-300">Sustitución de la velocidad:</p>
                            <p>v = √( 2 × {ffGravity} m/s² × {ffHeight} m )</p>
                            <p>v = √( {(2 * ffGravity * ffHeight).toFixed(2)} )</p>
                            <p className="text-amber-300 font-black text-xs">
                              ⟹ v = {Math.sqrt(2 * ffGravity * ffHeight).toFixed(2)} m/s ({(Math.sqrt(2 * ffGravity * ffHeight) * 3.6).toFixed(1)} km/h)
                            </p>
                          </div>
                          <p className="text-[10px] text-slate-400 italic">
                            {ffVacuum
                              ? '⚡ En vacío, el 100% de la energía potencial se transforma en energía de impacto.'
                              : '💨 En aire, la resistencia del aire disipa parte de la energía potencial en forma de fricción térmica y arrastre fluido.'}
                          </p>
                        </div>
                      </div>
                    </div>
